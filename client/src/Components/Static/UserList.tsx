                ...
                <div onClick={(e) => handelOpenUser(e, ite.uid)} key={ite.uid} className='flex flex-row justify-between items-start px-2 py-3 border-b-2 border-gray-200 '>
                    <div className="flex-row flex justify-between items-start ">
                        ...
                        <div className="flex flex-col justify-center items-center px-2 ">
                            <h5 className='text-md font-semibold'>{ite?.userName && ite?.userName.trim().substring(0, 10)}</h5>
                            <p className={`text-[14px] font-semibold ${onlineUsers?.some((usr) => usr.toString() === ite?.uid?.toString())
                                ? "text-green-400" : "text-red-300"}`}>
                                {onlineUsers?.some((usr) => usr.toString() === ite?.uid?.toString())
                                    ? "Online" : "Offline"}
                            </p>
                        </div>
                    </div>
